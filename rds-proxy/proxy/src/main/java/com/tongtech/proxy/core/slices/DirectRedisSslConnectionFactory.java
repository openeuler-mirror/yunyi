...
import static com.tongtech.proxy.core.slices.DirectRedisConnectionFactory.DIRECT_LOOP_GROUP_REDIS;

public class DirectRedisSslConnectionFactory {
            ...
            if (channel != null) {
                synchronized (channel) {
                    ...
                    if (properties != null) {
                        ...
                        Logger.debugLog("DirectRedisSslConnectionFactory::getConnection() channel ok at {}", channel.localAddress());
                        return channel;
                    } else {
                        Logger.infoLog("DirectRedisSslConnectionFactory::getConnection() null property at {}", channel.localAddress());
                    }
                }
            }
        } catch (Throwable t) {
            Logger.warnLog("DirectRedisSslConnectionFactory::getConnection() Get connection from {}:{} failed: {}"
