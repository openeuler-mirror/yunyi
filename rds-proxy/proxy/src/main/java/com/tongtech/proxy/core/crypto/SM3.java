...
public class SM3 {
    ...
    private static String byteToString(byte[] data) {
        ...
        for (int i = 0; data != null && i < data.length; i++) {
            buf.append(hexDigits[(byte) ((data[i] & 0xF0) >> 4)]);
            buf.append(hexDigits[(byte) (data[i] & 0x0F)]);
        }
        return buf.toString();
    }
    ...
    public static String hash(String source) throws Exception {
        return byteToString(hash(source.getBytes()));
    }

    public static byte[] hash(File file) throws Exception {
        if (file.exists()) {
            InputStream inStream = new FileInputStream(file);
            return hash(readInputStream2ByteArray(inStream));
        } else {
    ...
    private static byte[] readInputStream2ByteArray(InputStream inStream) throws IOException {
