...
create table sys_user (
  ...
  login_retries     int(9)          default 0                  comment '登录重试次数,登录成功后重置为0',
...
-- 初始化-用户信息表数据
-- ----------------------------
insert into sys_user values(1,  103, 'admin', '系统管理员', '00', 'admin@company.com', '16888888888', '1', '', '$2a$10$7JB720yubVSZvUI0rEqK/.VqGOZTH.ulu33dHOiBE8ByOhJIrdAu2', '0', '0', '127.0.0.1', CURRENT_TIMESTAMP(), 0, '0', CURRENT_TIMESTAMP(), 'admin', CURRENT_TIMESTAMP(), '', null, '系统管理员');
...
insert into sys_dict_type values(23, '登录锁定', 'sys_user_login_locked','0','admin', CURRENT_TIMESTAMP(), '', null, '登录锁定状态列表');
...
insert into sys_dict_data values(45, 1,  '正常',     '0',       'sys_user_login_locked',   '',   'info', 'N', '0', 'admin', CURRENT_TIMESTAMP(), '', null, '正常状态');
insert into sys_dict_data values(46, 2,  '锁定',     '1',       'sys_user_login_locked',   '',   'danger',  'N', '0', 'admin', CURRENT_TIMESTAMP(), '', null, '锁定状态');
...
insert into sys_config values(6, '登录重试次数',               'sys.login.maxRetries',          '6',         'Y', 'admin', CURRENT_TIMESTAMP(), '', null, '用户登录失败多少次后锁定账号');
insert into sys_config values(7, '密码过期时长(天)-过期后建议更换',    'sys.password.expireDays', '30',       'Y', 'admin', CURRENT_TIMESTAMP(), '', null, '建议用户更换密码时间间隔(单位天)');
...
DROP TABLE IF EXISTS sys_object_cache;
CREATE TABLE `sys_object_cache` (
    `obj_key` varchar(1024) NOT NULL,
    `obj_value` blob NOT NULL,
    `create_time` bigint(20) NOT NULL,
    `expire_time` bigint(20) DEFAULT NULL,
    PRIMARY KEY (`obj_key`)
) ENGINE=InnoDB comment = '对象缓存表';
