...
**节点：** 即TongRDS-CN的节点，分为工作节点、哨兵节点和代理节点两种类型．
...
在修改界面中，部署环境要选为"k8s容器云","服务地址"请填写中心服务的访问地址，"管理关口"是中心服务的管理端口。
...
在线用户提供查看当前登录管控台的用户以及对强制退出登录用户等
...
系统中所有的新建、修改、变更、删除等操作的历史都会被操作日志记录下来，且可以通过系统审计》操作日志 界面进行查询、查看和导出操作。
