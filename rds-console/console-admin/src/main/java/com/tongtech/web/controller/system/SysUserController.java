 ...
 */
@RestController
@RequestMapping("/web-api/system/user")
public class SysUserController extends BaseController {
    ...
    public AjaxResult resetPwd(@RequestBody SysUser user) {
        ...
        if (i > 0) {
            ...
            passwordService.renewPasswordExpiredTime(user.getUserId());
    ...
    @PreAuthorize("@ss.hasPermi('system:user:edit')")
    @Log(title = "用户管理-登录解锁", businessType = BusinessType.CLEAN)
    @GetMapping("/unlock/{userId}")
    public AjaxResult unlock(@PathVariable("userId") Long userId)
    {
        passwordService.clearLoginLocked(userId);
        return success();
    }
